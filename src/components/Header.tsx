    ...
    <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-navy-dark/95 backdrop-blur-md shadow-lg' : 'bg-navy-dark/80 backdrop-blur-sm'}`}>
      <nav className="container flex justify-between items-center h-20">
        ...
        >
          ...
          <span 
            className="hidden lg:block font-serif text-lg xl:text-xl font-bold gradient-text-gold"
            ...
            <li key={item.href}>
              <a 
                ...
                className={`relative font-medium py-2 transition-colors ${
                  activeSection === item.href.substring(1) ? 'text-gold-bright' : 'text-cream-light hover:text-gold-bright'
                }`}
              >
                {item.label}
                <span className={`absolute bottom-0 left-0 h-0.5 bg-gradient-to-r from-gold to-gold-bright transition-all duration-300 ${
