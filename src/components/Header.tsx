  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-navy-dark/95 backdrop-blur-md shadow-lg' : 'bg-navy-dark/80 backdrop-blur-sm'}`}>
      <nav className="container flex justify-between items-center h-24">
        ...
        {/* Logo y Nombre: CORREGIDO para que no desaparezca y use el degradado */}
        <a 
          href="#home" 
        ...
        >
          ...
          <span 
            className="hidden lg:block font-serif text-xl xl:text-2xl font-bold text-white transition-all duration-300 group-hover:gradient-text-gold group-hover:text-transparent"
        ...
        {/* Desktop Navigation: SE MANTIENE IGUAL A TU CÓDIGO */}
              ...
              <li key={item.href}>
                <a 
                  ...
                  className={`relative font-medium py-2 text-lg transition-all duration-300 inline-block cursor-pointer
                    ${isActive ? 'gradient-text-gold text-transparent font-bold scale-105' : 'text-white hover:gradient-text-gold hover:text-transparent hover:scale-105'}`}
                >
                  {item.label}
                  ...
                  >
                    <a 
                      ...
                      className={`text-2xl font-medium transition-all duration-300 ${
                        activeSection === item.href.substring(1) ? 'gradient-text-gold text-transparent' : 'text-white'
                      }`}
                    >
                      {item.label}
                    </a>
